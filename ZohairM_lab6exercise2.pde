...
void draw()
{
     ...
     // direction of a
     double mag = a.norm2();
     Matrix a_unit = a.times(1/mag);
     // STEP2: Insert code here to compute the dot product of b and a_unit
     Matrix projection = a_unit.transpose().times(b);
     double dot_product = projection.get(0,0);
     // STEP3: Insert code here to compute the vector p using equation 1.5 above
     Matrix p = a_unit.times(dot_product);
     ...
     double mag2 = c.norm2();
     Matrix c_unit = c.times(1/mag2);
     Matrix projection2 = c_unit.transpose().times(b);
     double dot_product2 = projection2.get(0,0);
     Matrix q = c_unit.times(dot_product2);
     
     float cx = (float)c.get(0,0);
     float cy = (float)c.get(1,0);
     float qx = (float)q.get(0,0);
     float qy = (float)q.get(1,0);
     
     stroke(0,0,0);             // Use a black pen
     ellipse(X+qy,Y-qx,10,10);  // point where b projects onto c
     line(X+qy,Y-qx,X+bx,Y+by); // line from c to point of projection on b
     
     stroke(0,0,255);     // Make pen blue
     arrow(X,Y,X+cy,Y-cx); // Draw vector 90 degrees(counter-clockwise) to a, starting at (X,Y)
